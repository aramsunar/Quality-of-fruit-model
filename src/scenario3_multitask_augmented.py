...
This scenario applies data augmentation techniques to colored images including
rotation, flipping, color jittering, and affine transformations. This helps
evaluate whether augmentation improves multi-task model generalization and
reduces overfitting for both fruit type and quality classification.
"""
...
class Scenario3MultitaskConfig:
    """Configuration for Scenario 3 Multi-Task."""

    # Scenario info
    SCENARIO_NAME = "scenario3_multitask_augmented"
    SCENARIO_DESCRIPTION = "Multi-task with data augmentation on colored images"
...
def run_scenario3_multitask(config=None):
    """
    Run Scenario 3 (Multi-Task): Data augmentation on colored images.

    Args:
        config: Configuration object (default: Scenario3MultitaskConfig)

    Returns:
        dict: Results including metrics and history
    """
    if config is None:
        config = Scenario3MultitaskConfig()

    print_section("SCENARIO 3 (MULTI-TASK): DATA AUGMENTATION")
    ...
    print(f"\nQuality classes ({num_quality_classes}): {', '.join(quality_classes)}")
    ...
    print_section("SCENARIO 3 (MULTI-TASK) COMPLETED", 50)
...
if __name__ == "__main__":
    results = run_scenario3_multitask()
    if results:
        print("\nScenario 3 (Multi-Task) completed successfully!")
        ...
        print("\nCompare this with Scenario 1 (Multi-Task) to assess the impact of data augmentation on both tasks.")
